...
const SeatVisualizer = ({ boatData, date, location, place, time, onSwap, isEditable = false, showStats = false }) => {
  ...
  // 優先使用 location (MemberPage用)，如果沒有則使用 place (CoachPage用)
  ...
  const Seat = ({ user, side, index }) => {
    const isSelected = selectedSeat?.side === side && selectedSeat?.index === index;
    
    return (
      <div 
        onClick={() => handleSeatClick(side, index)}
        className={`
          flex flex-col items-center justify-center 
          w-20 h-20 md:w-24 md:h-24 
          border-2 rounded-lg shadow-sm m-1 transition-all
          ${side === 'left' ? 'bg-blue-50 border-blue-200' : 'bg-green-50 border-green-200'}
          ${user?.weight > 80 ? 'border-red-400 border-4' : ''}
          ${isSelected ? 'ring-4 ring-orange-500 scale-110 z-20' : ''} 
  ...
  return (
    <div className="flex flex-col items-center w-full my-6">
      ...
      <div className="flex flex-col items-center gap-2 mb-4">
        ...
        {isEditable && (
            <div className="text-orange-600 font-bold bg-orange-50 px-4 py-1 rounded-full border border-orange-200 text-sm">
            👆 點擊兩個位置可交換隊員 (Manual Adjustment)
            </div>
        )}
      </div>
      ...
      <div 
        ...
        style={{ 
             backgroundImage: 'radial-gradient(circle at center, white 0%, #f0f9ff 100%)' 
        }}
      >
           ...
           {Array.from({ length: 10 }).map((_, i) => {
             ...
             return (
               <div key={rowIndex} className="flex gap-4 items-center justify-center relative">
                  ...
                  <Seat user={boatData.left[rowIndex]} side="left" index={rowIndex} />
                  <div className="text-sky-300 font-mono w-8 text-center font-black text-xl italic drop-shadow-sm">
                    {rowIndex + 1}
                  </div>
                  <Seat user={boatData.right[rowIndex]} side="right" index={rowIndex} />
               </div>
             );
           })}
        ...
        <div className="mt-6 relative z-10">
           ...
           <div className="w-24 h-24 bg-gradient-to-br from-yellow-50 to-yellow-100 border-4 border-yellow-400 rounded-full flex flex-col items-center justify-center shadow-lg relative transform hover:scale-105 transition">
             <span className="font-black text-xs text-yellow-700 mb-1 uppercase tracking-widest">Steer 舵手</span>
             <span className="font-extrabold text-lg text-gray-800">{boatData.steer ? boatData.steer.Name : "?"}</span>
           </div>
        </div>
